(() => {
  ...
  function updateFacetActiveChip(filter) {
    ...
    if (filter === 'all') {
      ...
      facetActiveChip.setAttribute('aria-label', t('facets.clearFilterIdle'));
      return;
    }
  ...
  function renderDrawer(project) {
    ...
    introActions.innerHTML = `
      <a class="drawer__visit" href="${esc(project.url)}">
    ...
    const actionLinks = [];
    if (project.github) {
      actionLinks.push(`<a class="btn btn--ghost" href="${esc(project.github)}">
  ...
  function closeVideoModal() {
    ...
    if (videoIframe) {
      videoIframe.src = '';
      videoIframe.title = t('videoModal.defaultTitle');
    }
    if (videoTitle) videoTitle.textContent = t('videoModal.defaultTitle');
    unlockBodyScroll();
